...
- [x] Workspace: rename the empty crate, add the CLI target, centralize only
  bitcoin/clap/serde/serde_json/thiserror, resolve Cargo.lock, run workspace check.
- [x] Script classification: in `transaction/script.rs`, test canonical P2PKH,
...
- [x] Explicit RBF: in `transaction/rbf.rs`, test sequence values 0, 0xfffffffd,
  0xfffffffe and 0xffffffff; implement `signals_explicit_rbf(u32) -> bool`.
- [x] Parsing: add `error.rs`, `limits.rs` and `transaction/analyzer.rs`. Test
...
- [x] Reports: add `transaction/report.rs` and the public
...
- [x] Human CLI: implement clap commands in `main.rs` and output in `display.rs`.
...
- [x] JSON CLI: add `--json`, serialize only the report to stdout, and test with
  serde_json including invalid-input behavior. Handle output I/O errors.
- [x] Adversarial review: exercise malformed short byte sequences, impossible
...
- [x] Documentation and verification: update README capabilities, examples,
  ...
  commit. Mark only Milestone 1 complete once verified. Branch publication follows
  verification using a normal push, with no PR.
...
54 tests passed, with fmt, all-target/all-feature check and Clippy passing.
CLI help, version, legacy, SegWit and JSON examples were verified. Independent
local code and documentation reviews found no blocking issues. The extra
100,000-item aggregate witness report limit prevents disproportionate report
allocation and is documented as an implementation limit, not a consensus rule.
See `milestone-1-verification.md` for evidence and remaining limitations.
